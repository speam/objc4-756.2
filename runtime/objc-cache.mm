...
static void cache_fill_nolock(Class cls, SEL sel, IMP imp, id receiver)
{
    ...
    // Never cache before +initialize is done   在+initialize完成之前永远不要缓存
    ...
    // 三种策略
    if (cache->isConstantEmptyCache()) {    // ①初始化缓存
        // 当第一次调用类的实例方法时（如本文的【1.2】例中的alloc init还没被调用的时候）
        // Cache is read-only. Replace it.
        cache->reallocate(capacity, capacity ?: INIT_CACHE_SIZE);
    }
    else if (newOccupied <= capacity / 4 * 3) { // ②直接缓存
    ...
    // 根据sel寻找对应的bucket，此bucket的sel一般为0（说明这个位置还没缓存方法），
