...
struct cache_t {
    struct bucket_t *_buckets;   // 缓存数组，即哈希桶        8字节
    mask_t _mask;                // 缓存数据的容量临界值       4字节
    mask_t _occupied;            // 缓存数组中已缓存方法数量    4字节
...
struct protocol_list_t {
    ...
    uintptr_t count;
    protocol_ref_t list[0]; // variable-size（可变大小）
...
struct objc_class : objc_object {   // 类有4个成员变量：1.isa，2.superclass，3.cache，4.bits
    // Class ISA;      // 8 字节
